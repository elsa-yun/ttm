...
public abstract class QuartzJobBean implements Job {
	...
	public final void execute(JobExecutionContext context) throws JobExecutionException {
		...
		String job_environment_type = (String) context.getMergedJobDataMap().get("job_environment_type");

		if (job_environment_type.equals(DynamicActionDO.JOB_ENVIRONMENT_TYPE_SINGLE)) {// 单节点任务需要lock
			long start = System.currentTimeMillis();
			boolean hasLock = clientManager.lock(ScheduleManager.JOB_GROUP_NAME, context.getJobDetail().getName());
			long end = System.currentTimeMillis();
			String str = "get lock start=>" + start + ";end=>" + end + ";hasLock=>" + hasLock;
			context.getJobDetail().getJobDataMap().put("lock_msg", str);
			if (hasLock) {
				try {
					runExecute(context);
				} finally {
					clientManager.unLock(ScheduleManager.JOB_GROUP_NAME, context.getJobDetail().getName());
				}
			}
		} else {
			context.getJobDetail().getJobDataMap().put("lock_msg", "not lock");
			runExecute(context);
		}
	}

	private void runExecute(JobExecutionContext context) throws JobExecutionException {
		try {
			// Reflectively adapting to differences between Quartz
			// 1.x
			// and
			// Quartz 2.0...
